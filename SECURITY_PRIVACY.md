...
- Secrets: platform secret managers; quarterly rotation note in ops docs; Anthropic/Voyage/Brevo keys scoped and budget-capped.
...
| **Processor inventory & DPAs** | Supabase (EU; DB/auth/storage) · Vercel (hosting/CDN) · Railway (worker) · Anthropic (pipeline text; **no user personal data in pipeline inputs by design** — the pipeline processes news, not user content; the only user text reaching Anthropic in V1 is *none* — duplicate detection uses Voyage embeddings) · Voyage AI (argument text embeddings — pseudonymous content, no identifiers attached) · Brevo (EU — email addresses + minimal delivery payloads only; founder rule: no opinion content in Brevo metadata, tags, contact attributes, or analytics fields) · Sentry EU (scrubbed telemetry) · GitHub (code only). DPAs + SCCs required for each US processor; inventory published in the privacy policy. **With Brevo (EU) as the email processor, the remaining US transfers to scrutinize are Anthropic (news text) and Voyage (pseudonymous argument text) (§8-Q4); a self-hosted embedding model is the identified fallback if counsel objects to Voyage.** |
...
4. **Transfers**: SCC adequacy for Anthropic (news-text only) and Voyage (pseudonymous argument text); whether EU-resident alternatives should be mandated (email is already EU-resident via Brevo).
